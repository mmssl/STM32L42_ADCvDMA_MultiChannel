...
void ADC_Init (void)
{
  ...
  ADC1->SMPR1 &= ~(7<<0) & ~(7<<3); // reset Sampling rate for channel 0 & 1
  ADC1->SMPR1 |= (7<<0) | (7<<3); // 640.5 Sampling rate for channel 0 & 1
  ADC1->SQR1 &= ~(0xf);
  ADC1->SQR1 |= (1<<0); // for 2 channel

  GPIOC->MODER &= ~(0xffffffff); // reset the whole register
  GPIOC->MODER |= (3<<0) | (3<<2); // set the analog mode to pin PC0 and PC1
